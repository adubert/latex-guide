% ========= Document Class ============
%   - Contains Preamble and assortment 
% 	  of other macros (commands)
% =====================================
\input{docpreamble} 
% ========= Path to images ============
...
% =================
	...
	\includegraphics[scale=0.17]{bim}	
	\vfill
...
\vspace{10.5cm}
	...
	\subsection{Source Code and Resume Templates}
			If interested in obtaining the source code for this guide, see:\url{https://sourceforge.net/p/latex-source-code/wiki/Download/}\\
...
The following provides you with the preamble/formatting for lessons and exams as well as how to organize your code and document.
	\subsection{Lessons} 
		\begin{docCommand}{input}{\brackets{lesson}}
				This command provides all the formatting you need to create a lesson document.   
		\end{docCommand}
	\subsection{Exams}
		\begin{docCommand}{input}{\brackets{examMC}}
				This command provides all the formatting you need to create a Multiple Choice Exam. On how to add questions and fully write a Multiple Choice Exam, see \fullref{writingexams} and \fullref{exams}
		\end{docCommand}
		\begin{docCommand}{input}{\brackets{examFR}}
				This command provides all the formatting you need to create a Free Response Exam. On how to add questions and fully write a Free Response Exam, see \fullref{writingexams} and \fullref{exams}
		\end{docCommand}
...
\vspace{10.5cm}
	...
	\subsection{Centering}
		...
		\begin{verbatim}\centerline{This line will be centered}.\end{verbatim}
		\subsubsection{Multi-Line}
			If you have several lines to be centered horizontally, the center environment is convenient. Make sure when using this environment, to use the line break, see- \ref{linebreaking}
	...
	\subsection{Spacing and New Pages}
		\subsubsection{Inserting Horizontal and Vertical Space}
			...
			\end{docCommand}
		\subsubsection{Page and Line Breaks}\label{linebreaking}
...
\section{Tests/Exams}	
\vspace{10.5cm}	
	\subsection{Math Environments}\label{mathenvironments}
	...
	\subsection{Structuring}
		...
		\end{docCommand}
	\subsection{Writing Exams}\label{writingexams}
		\subsubsection{Questions}
			...
			\end{docCommand}
			\begin{docCommand}{vspace}{\brackets{\cs{stretch}\brackets{1}}}
	...
	\subsubsection{Displaying Solutions}
		...
		\end{verbatim} which you can find by going to the examMC .tex file within the folder originally containing this document and adding [answers] at the top of the preamble at the document class or give the command \cs{printanswers}.\\ \vspace{1.5mm}
...
Macros are great for repetitive elements and texts that you carry across several documents.
	\subsection{Main Box Environments}
		...
		\subsubsection{Boxes for Visualizations}
			...
			\end{docEnvironment}
	\subsection{Extra Box Environments}
		\begin{docEnvironment}{example2}{\brackets{\sl{title}}}
			Another option for an example box.
		\end{docEnvironment}
...
\vspace{10.5cm}
	...
	\subsection{Easiest Way to Insert Graphics - w/ More Features}
		Another easy way to insert graphics that also provides more features is to go to the taskbar that should be located at the top of your screen and click the dropdown mean labeled \textbf{Wizards}. Once there, go to the 6th one down, the bottom one, labeled \textbf{Insert Graphic}. Once there, find where you put the image you wanted to insert into your document and then add any captions/labels/or other customization that you want to add with your graphic and then press ok. 
	...
	Most of the time, it will easier to insert images into a file using the method we mentioned in the previous subsection. However, the following allows for a cleaner, more organized, but a little more complicated at times possibly more advanced way to insert images into your \LaTeX\ file.
		\begin{docCommand}{graphicspath}{\brackets{path}}
			...
			\end{verbatim}
		\end{docCommand}
	\subsection{Inserting Images - w/ Graphics Path}
...
\vspace{10.5cm}
	\subsection{References/Hyperlinks and Formatting}
		...
		\end{docCommand}
		\begin{verbatim}
			\subsection{Math Tests/Exams and Math Symbols/Functions}\label{exams}
		\end{verbatim}
...
To access the library, go to the bottom left corner of \LaTeX\ where one will see the following boxes:\\	
	\begin{center}
		\includegraphics[scale=0.6]{BottomLeftCorner.png}
	\end{center}
...
If there are some symbols that you regularly and are not contained in that library, check the following symbols in this section or see: \url{https://www.caam.rice.edu/~heinken/latex/symbols.pdf}
	...
	For a comprehensive and detailed guide for Math Functions, see:\\ \url{https://artofproblemsolving.com/wiki/index.php/LaTeX:Commands}
		\begin{docCommand}{sqrt}{\brackets{abd}}
			$\sqrt{abd}$
		\end{docCommand}
		\begin{docCommand}{sqrt}{[n]\brackets{abd}}
			$\sqrt[n]{abd}$
		\end{docCommand}
	\subsection{Limits, Integrals, Sums, and Fractions}
		...
		\begin{docCommand}{int}{}
			\begin{verbatim}
				$\int_{lower}^{upper}$
			\end{verbatim}
			This is the command for creating integrals. Make sure when using to put it in a Math Environment, see \fullref{mathenvironments} or put dollar signs on both sides of the integral.
		\end{docCommand}
		\begin{docCommand}{dfrac}{\brackets{\sl{abc}}\brackets{\sl{xyz}}}{}
			...
			\end{verbatim}
			$\dfrac{abc}{xyz}$
			This is the command for creating Fractions. Make sure when using to put it in a Math Environment, see \fullref{mathenvironments} or put dollar signs on both sides of the fraction.
		\end{docCommand}
		\begin{docCommand}{sum}{}
			\begin{verbatim}
				$\sum_{lower}^{upper}$
			\end{verbatim}
			This is the command for creating Summations. Make sure when using to put it in a Math Environment, see \fullref{mathenvironments} or put dollar signs on both sides of the Summation.
		\end{docCommand}	
		\begin{docCommand}{lim}{}
			...
			\end{verbatim}
			$$\lim_{x\to\infty} f(x)$$
			This is the command for creating Limits. Make sure when using to put it in a Math Environment, see \fullref{mathenvironments} or put two dollar signs on both sides of the limit.
		\end{docCommand}	
	...
	\subsection{Boxes}
		...
		\begin{itemize}
			\item tcolorbox.pdf
				...
				\end{absquote}
		\end{itemize}
	\subsection{Typesetting Math Documents}\label{exams}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	\vspace{10.5cm}
		...
		\bibitem{texstackexchange}	Tex Stack Exchange. How to change chapter/section style in tufte-book?.\\
